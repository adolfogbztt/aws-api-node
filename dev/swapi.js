...
const replaceOriginalLink = (body, url = 'localhost:8000') => {
...
const traducir = (body) => {
    ...
    const keys_english = Object.keys(body);
    const string_keys_english = keys_english.toString();
    console.log(keys_english);

    var params = {
        SourceLanguageCode: 'en',
        TargetLanguageCode: 'es',
        Text: string_keys_english
    };
    ...
    // });  
    let data = []
    data['TranslatedText'] = 'aji, comino'


    const translate = removeAccentsAndSpecialChars(data['TranslatedText']);
    const string_spanish_to_keys_array = translate.split(", ").map((v, k) => v.replaceAll(' ', '_'));


    const body_translate = {};

    for (let i = 0; i < keys_english.length; i++) {
        body_translate[string_spanish_to_keys_array[i]] = body[keys_english[i]]
    }

    replaceOriginalLink(body_translate);


    // obtener valores de los arrays
    ...
    const obj = modificarClaves(body)
